...
@click.option('--core-only', is_flag=True, help='Deploy only core agents (Maestro and Victoria)')
...
def status(customer_id: Optional[str], detailed: bool):
    ...
    console.print(Panel.fit(
        "[bold green]📊 Symphony Agent Status[/bold green]",
        border_style="green"
    ))
...
@click.option('--customer-id', '-c', help='Customer ID')
def handoff(from_agent: str, to_agent: str, objective: str, context: Optional[str], customer_id: Optional[str]):
...
@click.option('--agents', '-A', help='Multiple agents for coordination (comma-separated)')
@click.option('--data', '-d', help='Task data (JSON)')
@click.option('--customer-id', '-c', help='Customer ID')
def execute(task_type: str, agent_id: Optional[str], agents: Optional[str], data: Optional[str], customer_id: Optional[str]):
    ...
    if agents:
        ...
        console.print(f"[blue]⚙️ Coordinating task:[/blue] {task_type} across {len(agent_list)} agents")
        asyncio.run(_coordinate_multi_agent_task(task_type, agent_list, task_data, customer_id))
    elif agent_id:
...
def monitor(customer_id: Optional[str], live: bool, interval: int):
    ...
    if live:
        console.print("[blue]🔍 Starting live agent monitoring (Press Ctrl+C to stop)[/blue]")
...
def health(customer_id: Optional[str], format: str):
    ...
    console.print(Panel.fit(
        "[bold green]🏥 Agent Health Check[/bold green]",
        border_style="green"
    ))
    ...
    try:
        ...
        if core_only:
            ...
            with Progress(
                ...
                progress.update(deploy_task, completed=True, description="Core agents deployed")
            ...
            with Progress(
                ...
                deploy_task = progress.add_task(f"Deploying {package} package...", total=None)
                
                results = await manager.deploy_agent_package(package)
                
                progress.update(deploy_task, completed=True, description=f"{package} package deployed")
...
async def _show_agent_status(customer_id: Optional[str], detailed: bool):
    ...
    try:
        ...
        if 'system_metrics' in status:
            ...
            table.add_row("Handoff Success Rate", f"{metrics.get('handoff_success_rate', 0):.1f}%")
        ...
        if detailed and 'agent_details' in status:
            ...
            for agent_id, agent_status in status['agent_details'].items():
                ...
                agent_table.add_row("Active Tasks", str(agent_status.get('active_tasks', 0)))
                
                if 'metrics' in agent_status:
                    metrics = agent_status['metrics']
                    agent_table.add_row("Success Rate", f"{metrics.get('success_rate', 0):.1f}%")
                    agent_table.add_row("Total Tasks", str(metrics.get('total_tasks', 0)))
        ...
        for category, agents in available_agents.items():
            if agents:
                ...
                for agent_info in agents:
                    status = "Available" if agent_info.get('available', False) else "Unavailable"
                    table.add_row(
...
async def _initiate_handoff(from_agent: str, to_agent: str, context_data: Dict[str, Any], customer_id: Optional[str]):
    ...
    try:
        ...
        if success:
            console.print(f"[green]✓[/green] Handoff completed: {from_agent} → {to_agent}")
        else:
...
async def _execute_single_agent_task(agent_id: str, task_data: Dict[str, Any], customer_id: Optional[str]):
...
async def _coordinate_multi_agent_task(task_type: str, agents: list, task_data: Dict[str, Any], customer_id: Optional[str]):
    ...
    try:
        ...
        if result.get('success', False):
            ...
            console.print(f"Coordination type: {result.get('coordination_type', 'unknown')}")
        else:
...
async def _health_check(customer_id: Optional[str], format: str):
    ...
    try:
        ...
        if format == 'json':
            ...
            health_table.add_row("System", f"[{health_color}]{overall_health}[/{health_color}]", f"{status['active_agents']}/{status['total_agents']} agents active")
