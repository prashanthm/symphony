...
def configure(config: Optional[str], interactive: bool, output: Optional[str], preview: bool):
    ...
    try:
        if interactive:
            ...
            console.print("[red]Error:[/red] Either --config or --interactive must be specified")
            raise click.Abort()
...
def generate(customer: str, industry: Optional[str], size: Optional[str], 
            regions: Optional[str], output: Optional[str], preview: bool):
...
def preview(config_file: str, detailed: bool):
    ...
    console.print(Panel.fit(
        "[bold cyan]👀 Workspace Preview[/bold cyan]",
        border_style="cyan"
    ))
...
@hierarchy.command()
@click.argument('config_file', type=click.Path(exists=True))
@click.option('--linear-token', envvar='LINEAR_API_TOKEN', required=True,
...
def deploy(config_file: str, linear_token: str, dry_run: bool, force: bool):
    ...
    try:
        ...
        if not result.is_valid:
            console.print("[red]❌ Configuration has errors. Please fix before deploying:[/red]")
            for error in result.errors:
                console.print(f"  [red]•[/red] {error}")
            raise click.Abort()
        ...
        if dry_run:
            ...
            console.print("[green]✓[/green] Configuration is valid and ready for deployment")
            return
...
def dogfood():
    ...
    try:
        ...
        console.print("[green]✓[/green] Generated Symphony internal workspace configuration")
...
def list_templates():
    ...
    # Industry templates
    table.add_row("Industry", "financial-services", "Banking and financial institutions")
...
def _display_generation_summary(template):
    ...
    if template.symphony_integration:
        total_agents = sum(
            len(agents) for agents in template.symphony_integration.agent_assignments.values()
        )
...
def _display_dogfooding_preview(template):
    ...
    table.add_row("Recursive Improvement", "✅ Enabled", "System improves its own templates")
