...
@click.option('--package', '-p', type=click.Choice(['startup', 'smb', 'enterprise', 'global']), help='Agent package to deploy')
        ...
        if package:
            ...
            async def run_deployment():
                ...
                if customer_id:
                    ...
                    if result['success']:
                        console.print(f"✅ {package} agents deployed successfully for {customer_id}")
                        console.print(f"📊 Agents deployed: {result['agents_deployed']}")
                    else:
                        console.print(f"[red]❌ Deployment failed: {result['error']}[/red]")
                else:
                    console.print("[yellow]⚠ No customer ID specified, deploying to default configuration[/yellow]")
                    result = await deploy_core_agents(manager)
                    console.print(f"✅ Core agents deployed: {result}")
            
            asyncio.run(run_deployment())
        else:
            console.print("[yellow]Please specify a package type: startup, smb, enterprise, or global[/yellow]")
    
    except ImportError as e:
        console.print(f"[red]❌ Import error: {e}[/red]")
        console.print("[yellow]Please ensure Symphony packages are properly installed[/yellow]")
        ...
        async def show_status():
            ...
            if customer_id:
                ...
                if status:
                    ...
                    console.print(f"[yellow]No agent status found for customer: {customer_id}[/yellow]")
            else:
...
def handoff(from_agent: str, to_agent: str, context: Optional[str], user_objective: Optional[str]):
    """Execute handoff between agents"""
    try:
        from symphony_core.agents.agent_manager import create_agent_manager
        
        console.print(f"[purple]🔄 Executing handoff: {from_agent} → {to_agent}[/purple]")
    ...
    try:
        ...
        async def run_task():
            ...
            if result['success']:
                ...
                console.print(f"⏱ Execution time: {result.get('execution_time', 'N/A')}s")
    ...
    try:
        ...
        async def run_monitoring():
            ...
            while time.time() - start_time < duration:
                ...
                console.clear()
                console.print(f"[blue]📊 Agent System Monitoring[/blue] - {click.datetime.datetime.now().strftime('%H:%M:%S')}")
                ...
                for metric, value in metrics.items():
                    status = "✅ OK" if isinstance(value, (int, float)) and value > 90 else "⚠️ Check"
