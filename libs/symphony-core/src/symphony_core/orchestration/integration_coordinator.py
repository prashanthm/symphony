        ...
        if event_type in self.event_handlers:
            for handler in self.event_handlers[event_type]:
                try:
                    ...
                    logger.error(
                        f"Event handler error for {self.integration_name}: {e}"
                    )
...
class IntegrationOrchestrator:
    ...
    async def register_integration(self, adapter: BaseIntegrationAdapter) -> bool:
        ...
        try:
            ...
            if await adapter.initialize():
                ...
                logger.info(
                    f"Integration {adapter.integration_name} registered successfully"
                )
                return True
            else:
                logger.error(
                    f"Failed to initialize integration {adapter.integration_name}"
                )
                return False

        except Exception as e:
            logger.error(
                f"Error registering integration {adapter.integration_name}: {e}"
            )
            return False
    ...
    async def trigger_orchestration(
        self, trigger_event: str, context: Dict[str, Any] = None
    ) -> Dict[str, Any]:
        ...
        for integration_name, adapter in self.integrations.items():
            if adapter.status == IntegrationStatus.ACTIVE:
                try:
                    ...
                    sync_results[integration_name] = {"success": True, "result": result}
                except Exception as e:
                    logger.error(f"Sync failed for {integration_name}: {e}")
                    sync_results[integration_name] = {"success": False, "error": str(e)}
            else:
        ...
        try:
            ...
            for i, step in enumerate(steps):
                ...
                if not result.get("success", True) and step.get("required", True):
                    ...
                    raise Exception(
                        f"Required step {i} failed: {result.get('error', 'Unknown error')}"
                    )
    ...
    async def _handle_data_sync_event(self, event: IntegrationEvent):
        ...
        logger.info(
            f"Data sync event from {event.integration_name}: {event.event_type}"
        )
    ...
    async def _handle_error_event(self, event: IntegrationEvent):
        """Handle error events"""
        logger.warning(
            f"Error event from {event.integration_name}: {event.data.get('error', 'Unknown error')}"
        )
    ...
    # Internal methods
    async def _execute_orchestration_rule(
        self, rule: OrchestrationRule, context: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        if rule.orchestration_type == OrchestrationType.SEQUENTIAL:
            ...
            for action in rule.actions:
                result = await self._execute_action(
                    action, rule.target_integrations, context
                )
        ...
        return {"success": success, "details": action_results}
...
async def create_data_sync_rule(
    integrations: List[str], sync_type: str = "bidirectional"
) -> OrchestrationRule:
    ...
    return OrchestrationRule(
        ...
        actions=[{"type": "sync_data", "params": {"sync_type": sync_type}}],
    )
