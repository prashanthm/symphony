...
class GitHubIntegrationAdapter(BaseIntegrationAdapter):
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            if not connection_test['success']:
                raise Exception(f"GitHub connection test failed: {connection_test.get('error')}")
    ...
    async def sync_data(self, data_type: str, options: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            self.health.success_rate = (self.metrics['sync_count'] - self.metrics['error_count']) / self.metrics['sync_count'] * 100
    ...
    async def get_health_status(self) -> IntegrationHealth:
        ...
        self.health.success_rate = (
            (self.metrics['sync_count'] - self.metrics['error_count']) / max(self.metrics['sync_count'], 1) * 100
        )
        ...
        try:
            ...
            if repository:
                ...
                for repo in repositories[:5]:  # Limit to prevent rate limiting
                    repo_issues = await self.github_client.get_repository_issues(repo['name'])
        ...
        try:
            ...
            if repository:
                ...
                for repo in repositories[:3]:  # Limit to prevent rate limiting
                    repo_workflows = await self.github_client.get_workflows(repo['name'])
    ...
    async def _update_issue(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            result = await self.github_client.update_issue(repository, issue_number, update_data)
    ...
    async def _merge_pull_request(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            result = await self.github_client.merge_pull_request(repository, pr_number, merge_data)
    ...
    async def _trigger_workflow(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            result = await self.github_client.trigger_workflow(repository, workflow_id, ref, inputs)
    ...
    async def _sync_with_symphony(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            sync_options = {
                ...
                'data_types': params.get('data_types', ['repositories', 'issues', 'pull_requests'])
            }
