...
class LinearAPIClient:
    ...
    async def get_teams(self) -> List[LinearTeam]:
        ...
        for team_data in result["teams"]["nodes"]:
            teams.append(
                LinearTeam(
                    id=team_data["id"], name=team_data["name"], key=team_data["key"]
                )
            )
    ...
    async def create_project(
        self, name: str, description: str, team_id: str
    ) -> LinearProject:
    ...
    async def update_issue(self, issue_id: str, updates: Dict[str, Any]) -> LinearIssue:
        ...
        variables = {"issueId": issue_id, "input": updates}
...
class SymphonyLinearIntegration:
    ...
    async def initialize_workspace(self, organization_name: str) -> Dict[str, Any]:
        ...
        async with self.client as client:
            ...
            config_file = (
                self.config_dir
                / f"{organization_name.lower().replace(' ', '_')}_linear_workspace.json"
            )
