...
from .template_models import (
    WorkspaceTemplate, OrganizationConfig, TeamTemplate, ProjectTemplate,
    Initiative, CustomField, WorkflowState, Milestone, SymphonyIntegration,
    IndustryType, OrganizationSize, FieldType
)
...
class SymphonyLinearDefaults:
    ...
    def generate_defaults(self, organization: OrganizationConfig) -> WorkspaceTemplate:
        ...
        template.symphony_integration = self._generate_symphony_integration(organization)
        
        return template
    
    def _add_industry_defaults(self, template: WorkspaceTemplate, industry: IndustryType) -> WorkspaceTemplate:
    ...
    def _add_finserv_defaults(self, template: WorkspaceTemplate) -> WorkspaceTemplate:
        ...
        compliance_workflows = self.base_workflows + [
            WorkflowState("Compliance Review", "started", 2.5, "Regulatory compliance review"),
            WorkflowState("Risk Assessment", "started", 2.7, "Risk analysis required"),
            WorkflowState("Regulatory Approval", "started", 3.5, "Awaiting regulatory approval")
        ]
        ...
        template.teams.extend([
            TeamTemplate(
                ...
                custom_fields=[
                    CustomField("Risk Level", FieldType.SELECT, 
                              options=["Low", "Medium", "High", "Critical"]),
                    CustomField("SOX Compliance", FieldType.BOOLEAN),
                    CustomField("Regulatory Framework", FieldType.SELECT,
            ...
            TeamTemplate(
                ...
                custom_fields=[
                    CustomField("Security Classification", FieldType.SELECT,
        ...
        template.initiatives.extend([
            Initiative(
                ...
                sub_initiatives=[
                    Initiative("SOX 404 Compliance", 2, "Sarbanes-Oxley compliance"),
                    Initiative("GDPR Enhancement", 2, "Enhanced GDPR compliance"),
                    Initiative("Cybersecurity Framework", 2, "NIST cybersecurity implementation")
                ]
            ),
            Initiative(
                ...
                sub_initiatives=[
                    ...
                    Initiative("Customer Experience Portal", 2, "Digital customer interface"),
                    Initiative("Real-time Risk Management", 2, "Automated risk assessment")
                ]
            )
        ])
    ...
    def _add_healthcare_defaults(self, template: WorkspaceTemplate) -> WorkspaceTemplate:
        ...
        hipaa_workflows = self.base_workflows + [
            ...
            WorkflowState("Privacy Assessment", "started", 2.7, "Privacy impact assessment"),
            WorkflowState("Clinical Approval", "started", 3.5, "Clinical stakeholder approval")
        ]
        ...
        template.teams.extend([
            TeamTemplate(
                ...
                custom_fields=[
                    CustomField("Patient Safety Impact", FieldType.SELECT,
            ...
            TeamTemplate(
                ...
                custom_fields=[
                    ...
                    CustomField("Compliance Framework", FieldType.SELECT,
        ...
        template.initiatives.extend([
            Initiative(
                ...
                sub_initiatives=[
                    Initiative("Electronic Health Records", 2, "EHR system implementation"),
                    Initiative("Telemedicine Platform", 2, "Remote care capabilities"),
                    Initiative("Patient Engagement Portal", 2, "Patient self-service platform")
                ]
            )
        ])
    ...
    def _add_technology_defaults(self, template: WorkspaceTemplate) -> WorkspaceTemplate:
        ...
        template.teams.extend([
            TeamTemplate(
                ...
                sub_teams=[
                    TeamTemplate("Frontend", "FE", "Frontend development", dev_workflows),
                    TeamTemplate("Backend", "BE", "Backend services", dev_workflows),
                    TeamTemplate("Infrastructure", "INFRA", "DevOps and infrastructure", dev_workflows)
                ],
                custom_fields=[
                    CustomField("Technical Complexity", FieldType.NUMBER, range=[1, 10]),
                    CustomField("Business Impact", FieldType.SELECT,
                              options=["Low", "Medium", "High", "Critical"]),
            ...
            TeamTemplate(
                ...
                workflows=self.base_workflows,
                custom_fields=[
                    CustomField("Customer Segment", FieldType.SELECT,
                              options=["Enterprise", "SMB", "Startup", "Individual"]),
                    CustomField("Feature Priority", FieldType.SELECT,
                              options=["P0", "P1", "P2", "P3"]),
                    CustomField("User Story Points", FieldType.NUMBER, range=[1, 21])
                ]
            )
        ])
        ...
        template.initiatives.extend([
            Initiative(
                ...
                sub_initiatives=[
                    Initiative("Microservices Architecture", 2, "Service decomposition"),
                    Initiative("Observability Platform", 2, "Monitoring and observability"),
                    Initiative("Developer Experience", 2, "Internal developer tools")
                ]
            )
        ])
    ...
    def _add_size_defaults(self, template: WorkspaceTemplate, size: OrganizationSize) -> WorkspaceTemplate:
        ...
        if not template.teams:
            template.teams.append(
                TeamTemplate(
                    ...
                    workflows=self.base_workflows,
                    custom_fields=[
                        CustomField("Priority", FieldType.SELECT, options=["High", "Medium", "Low"]),
                        CustomField("Effort", FieldType.SELECT, options=["XS", "S", "M", "L", "XL"])
                    ]
                )
            )
            ...
            template.initiatives.append(
                Initiative(
                    ...
                    sub_initiatives=[
                        ...
                        Initiative("Customer Validation", 2, "Validate product with customers"),
    ...
    def _add_enterprise_defaults(self, template: WorkspaceTemplate) -> WorkspaceTemplate:
            ...
            if not team.sub_teams:  # Add sub-teams to main teams
                if "Engineering" in team.name or "Platform" in team.name:
                    team.sub_teams.extend([
                        TeamTemplate("Architecture", "ARCH", "Technical architecture"),
        ...
        template.teams.append(
            TeamTemplate(
                ...
                custom_fields=[
                    CustomField("Architecture Review", FieldType.BOOLEAN),
                    CustomField("Enterprise Impact", FieldType.SELECT,
                              options=["Department", "Division", "Enterprise", "External"]),
                    CustomField("Governance Stage", FieldType.SELECT,
    ...
    def _add_generic_defaults(self, template: WorkspaceTemplate) -> WorkspaceTemplate:
        ...
        template.teams.extend([
            ...
            TeamTemplate(
                ...
                workflows=self.base_workflows,
                custom_fields=[
                    CustomField("Project Phase", FieldType.SELECT,
                              options=["Initiation", "Planning", "Execution", "Monitoring", "Closure"]),
                    CustomField("Budget Impact", FieldType.SELECT,
        ...
        if not any("Enterprise Architecture" in team.name for team in template.teams):
            template.teams.append(
                TeamTemplate(
                    ...
                    workflows=self.base_workflows,
                    custom_fields=[
                        CustomField("Region", FieldType.SELECT, 
                                  options=["AMER", "EMEA", "APAC", "Global"]),
                        CustomField("Compliance Region", FieldType.MULTI_SELECT,
                                  options=["US", "EU", "UK", "APAC", "Other"]),
                        CustomField("Priority Level", FieldType.SELECT,
    ...
    def _generate_symphony_integration(self, organization: OrganizationConfig) -> SymphonyIntegration:
        ...
        return SymphonyIntegration(
            ...
            automation={
                ...
                "status_sync": True,
                "reporting": True,
                "template_updates": organization.size in [OrganizationSize.ENTERPRISE, OrganizationSize.GLOBAL]
            },
            defaults_override={
    ...
    def generate_dogfooding_template(self) -> WorkspaceTemplate:
        ...
        template.teams = [
            TeamTemplate(
                ...
                workflows=self.base_workflows,
                sub_teams=[
                    TeamTemplate("Linear Integration", "LINEAR", "Linear workspace management"),
                    TeamTemplate("Agent Ecosystem", "AGENTS", "Agent development and coordination"),
                    TeamTemplate("Configuration Systems", "CONFIG", "Template and configuration systems")
                ]
            ),
            TeamTemplate(
                ...
                workflows=[
                    WorkflowState("Discovery", "unstarted", 1, "Customer needs analysis"),
                    WorkflowState("Implementation", "started", 2, "Symphony deployment"),
                    WorkflowState("Optimization", "started", 3, "Performance optimization"),
                    WorkflowState("Excellence", "completed", 4, "Autonomous excellence achieved")
                ]
            )
        ]
        ...
        template.initiatives = [
            Initiative(
                ...
                sub_initiatives=[
                    Initiative("Linear Integration Mastery", 2, "Perfect Linear workspace management"),
                    Initiative("Agent Ecosystem Excellence", 2, "85+ agent coordination perfection"),
                    Initiative("Customer Success Optimization", 2, "Perfect customer implementations")
                ]
            )
        ]
