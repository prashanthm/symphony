...
from .template_models import (
    WorkspaceTemplate, TemplateValidationResult, WorkspacePreview,
    TeamTemplate, ProjectTemplate, Initiative, CustomField,
...
class TemplateValidator:
    ...
    def __init__(self):
        ...
        self.reserved_keys = {
            'LINEAR', 'ADMIN', 'API', 'SYSTEM', 'ROOT', 'NULL', 'UNDEFINED'
        }
        self.max_team_hierarchy_depth = 3
        self.max_initiative_hierarchy_depth = 5
        self.max_team_key_length = 10
    
    def validate_template(self, template: WorkspaceTemplate) -> TemplateValidationResult:
        ...
        try:
            ...
            symphony_errors, symphony_suggestions = self._validate_symphony_integration(template)
    ...
    def _validate_business_rules(self, template: WorkspaceTemplate) -> tuple[List[str], List[str]]:
        ...
        for team in template.teams:
            ...
            if not team.key:
                ...
                errors.append(f"Team key '{team.key}' exceeds {self.max_team_key_length} characters")
            else:
            ...
            if team_depth > self.max_team_hierarchy_depth:
                errors.append(f"Team '{team.name}' hierarchy depth ({team_depth}) exceeds maximum ({self.max_team_hierarchy_depth})")
        ...
        for initiative in template.initiatives:
            ...
            if initiative_depth > self.max_initiative_hierarchy_depth:
                errors.append(f"Initiative '{initiative.name}' hierarchy depth ({initiative_depth}) exceeds maximum ({self.max_initiative_hierarchy_depth})")
        
        # Project validation
        project_names = set()
        for project in template.projects:
            if project.template_name in project_names:
                errors.append(f"Duplicate project template name '{project.template_name}'")
            ...
            for team_name in project.assignable_teams:
                if not any(team.name == team_name for team in template.teams):
                    errors.append(f"Project '{project.name}' references non-existent team '{team_name}'")
        
        return errors, warnings
    
    def _validate_linear_constraints(self, template: WorkspaceTemplate) -> tuple[List[str], List[str]]:
        ...
        if len(template.teams) > 50:
            warnings.append(f"Large number of teams ({len(template.teams)}) may affect performance")
        
        if len(template.projects) > 100:
            warnings.append(f"Large number of projects ({len(template.projects)}) may affect organization")
        
        # Custom field validation
        for team in template.teams:
            if len(team.custom_fields) > 20:
                warnings.append(f"Team '{team.name}' has many custom fields ({len(team.custom_fields)}) which may clutter the interface")
            
            for field in team.custom_fields:
                if field.type == FieldType.SELECT and field.options:
                    if len(field.options) > 50:
                        warnings.append(f"Select field '{field.name}' has many options ({len(field.options)})")
        
        # Workflow state validation
        for team in template.teams:
            if len(team.workflows) > 15:
                warnings.append(f"Team '{team.name}' has many workflow states ({len(team.workflows)}) which may complicate processes")
        
        return errors, warnings
    
    def _validate_symphony_integration(self, template: WorkspaceTemplate) -> tuple[List[str], List[str]]:
        """Validate Symphony-specific integration"""
        errors = []
        suggestions = []
        
        if not template.symphony_integration:
            suggestions.append("Consider adding Symphony integration for automated agent management")
        ...
        if integration.agent_assignments:
            ...
            if missing_teams:
                errors.extend([f"Agent assigned to non-existent team '{team}'" for team in missing_teams])
            
            unassigned_teams = template_teams - assigned_teams
            if unassigned_teams:
                suggestions.extend([f"Consider assigning agents to team '{team}'" for team in unassigned_teams])
        ...
        if integration.self_managing and template.organization:
            if template.organization.customer_name.lower() != "symphony":
                warnings = [f"Self-managing enabled for non-Symphony customer '{template.organization.customer_name}'"]
    ...
    def _suggest_best_practices(self, template: WorkspaceTemplate) -> List[str]:
        ...
        if len(template.teams) == 1:
            suggestions.append("Consider organizing work into multiple teams as your organization grows")
        
        # Initiative suggestions
        if not template.initiatives:
            suggestions.append("Consider adding initiatives to organize strategic goals")
        
        # Project template suggestions
        if not template.projects:
            suggestions.append("Consider adding project templates for consistent project setup")
        
        # Milestone suggestions
        for project in template.projects:
            if not project.milestones:
                suggestions.append(f"Consider adding milestones to project template '{project.template_name}'")
    ...
    def _calculate_initiative_depth(self, initiative: Initiative, current_depth: int = 1) -> int:
...
class WorkspacePreviewGenerator:
    ...
    def generate_preview(self, template: WorkspaceTemplate) -> WorkspacePreview:
        ...
        total_teams = team_count + sum(len(self._flatten_subteams(team)) for team in template.teams)
        ...
        # Get Symphony agents
        agents_deployed = []
        if template.symphony_integration and template.symphony_integration.agent_assignments:
        ...
        return WorkspacePreview(
            workspace_name=template.workspace.get('name', 'Unknown') if template.workspace else 'Unknown',
    ...
    def _calculate_complexity(self, template: WorkspaceTemplate) -> int:
        ...
        max_team_depth = max(
            (self._calculate_team_depth(team) for team in template.teams),
            default=1
        )
        if max_team_depth > 2:
            score += 2
        ...
        if template.initiatives:
            max_init_depth = max(
                (self._calculate_initiative_depth(init) for init in template.initiatives),
                default=1
            )
            if max_init_depth > 3:
                score += 1
    ...
    def _estimate_setup_time(self, template: WorkspaceTemplate) -> str:
        ...
        total_minutes = base_minutes + team_minutes + project_minutes + complexity_minutes
