...
class TestConfigureCommand:
    ...
    def test_configure_with_config_file(self, cli_runner, sample_config_file):
        ...
        with patch(
            "symphony_integrations.linear.template_engine.TemplateEngine"
        ) as mock_engine:
            mock_template = Mock()
            mock_engine.return_value.process_customer_config.return_value = (
                mock_template
            )

            result = cli_runner.invoke(
                hierarchy, ["configure", "--config", sample_config_file]
            )
    ...
    def test_configure_with_preview(self, cli_runner, sample_config_file):
        ...
        with patch(
            ...
            with patch(
                "symphony_cli.commands.linear_hierarchy._display_workspace_preview"
            ) as mock_preview:
                mock_template = Mock()
                ...
                result = cli_runner.invoke(
                    hierarchy,
                    ["configure", "--config", sample_config_file, "--preview"],
                )
    ...
    def test_configure_interactive(self, mock_wizard_class, cli_runner):
        ...
        with tempfile.NamedTemporaryFile(suffix=".yaml") as output_file:
            result = cli_runner.invoke(
                hierarchy, ["configure", "--interactive", "--output", output_file.name]
            )
    ...
    def test_configure_invalid_config_file(self, cli_runner, invalid_config_file):
        ...
        result = cli_runner.invoke(
            hierarchy, ["configure", "--config", invalid_config_file]
        )
...
class TestGenerateCommand:
    ...
    def test_generate_basic(self, cli_runner):
        ...
        with patch(
            "symphony_integrations.linear.defaults_generator.SymphonyLinearDefaults"
            ...
            result = cli_runner.invoke(
                hierarchy, ["generate", "--customer", "Test Corp"]
            )
    ...
    def test_generate_with_preview(self, cli_runner):
        ...
        with patch(
            ...
            with patch(
                ...
                result = cli_runner.invoke(
                    hierarchy, ["generate", "--customer", "PreviewCorp", "--preview"]
                )
...
class TestValidateCommand:
    ...
    def test_validate_valid_config(self, cli_runner, sample_config_file):
        ...
        with patch(
            ...
            with patch(
                "symphony_integrations.linear.template_validator.TemplateValidator"
    ...
    def test_validate_invalid_config(self, cli_runner, sample_config_file):
        ...
        with patch(
            ...
            with patch(
                ...
                assert (
                    result.exit_code == 0
                )  # Command should succeed even with validation errors
    ...
    def test_preview_detailed(self, cli_runner, sample_config_file):
        ...
        with patch(
            ...
            with patch(
                ...
                result = cli_runner.invoke(
                    hierarchy, ["preview", sample_config_file, "--detailed"]
                )
...
class TestDeployCommand:
    ...
    def test_deploy_requires_token(self, cli_runner, sample_config_file):
        ...
        assert result.exit_code != 0
        assert (
            "linear-token" in result.output.lower()
    ...
    def test_deploy_invalid_config_blocks_deployment(
    ...
    def test_deploy_simulation(self, cli_runner, sample_config_file):
        ...
        with patch(
            ...
            with patch(
                ...
                with patch(
                    "symphony_cli.commands.linear_hierarchy._simulate_deployment"
                ) as mock_simulate:
                    mock_template = Mock()
                    mock_engine.return_value.process_customer_config.return_value = (
                        mock_template
...
class TestDogfoodCommand:
    ...
    def test_dogfood_basic(self, cli_runner):
        ...
        with patch(
            ...
            with patch(
                "symphony_cli.commands.linear_hierarchy._display_dogfooding_preview"
            ) as mock_preview:
                mock_generator = Mock()
                mock_template = Mock()
                ...
                assert (
                    "Generated Symphony internal workspace configuration"
                    in result.output
                )
                mock_preview.assert_called_once()
        ...
        with patch(
            ...
            with patch(
                ...
                with patch(
                    ...
                    mock_generator = Mock()
                    mock_template = Mock()
                    mock_generator.generate_dogfooding_template.return_value = (
                        mock_template
                    )
                    mock_generator_class.return_value = mock_generator
...
class TestDisplayFunctions:
    ...
    def test_display_workspace_preview(self, mock_console):
        ...
        with patch(
            "symphony_cli.commands.linear_hierarchy.WorkspacePreviewGenerator"
        ) as mock_generator_class:
            mock_generator = Mock()
            mock_preview = Mock()
