...
from symphony_integrations.linear.template_models import (
    OrganizationConfig, WorkspaceTemplate, TeamTemplate, 
    IndustryType, OrganizationSize, FieldType, CustomField,
    WorkflowState, Initiative, ProjectTemplate
)
...
class TestBaseWorkflowGeneration:
    ...
    def test_base_workflows_structure(self, defaults_generator):
        ...
        expected_names = ["Backlog", "Todo", "In Progress", "Review", "Done", "Canceled"]
...
class TestDefaultGeneration:
    ...
    def test_generate_defaults_basic_structure(self, defaults_generator, sample_organizations):
    ...
    def test_generate_defaults_workspace_naming(self, defaults_generator, sample_organizations):
    ...
    def test_generate_defaults_includes_symphony_integration(self, defaults_generator, sample_organizations):
...
class TestIndustrySpecificDefaults:
    ...
    def test_financial_services_custom_fields(self, defaults_generator, sample_organizations):
        """Test financial services custom fields"""
        org = sample_organizations['finserv_startup']
        template = defaults_generator.generate_defaults(org)
        
        # Find risk management team
        risk_team = next(team for team in template.teams if team.name == "Risk Management")
        ...
        risk_field = next(field for field in risk_team.custom_fields if field.name == "Risk Level")
        assert risk_field.type == FieldType.SELECT
        assert "Critical" in risk_field.options
        assert "Low" in risk_field.options
    
    def test_financial_services_workflows(self, defaults_generator, sample_organizations):
        """Test financial services compliance workflows"""
        org = sample_organizations['finserv_startup']
        template = defaults_generator.generate_defaults(org)
        
        compliance_team = next(team for team in template.teams if team.name == "Regulatory Compliance")
    ...
    def test_financial_services_initiatives(self, defaults_generator, sample_organizations):
        ...
        compliance_init = next(init for init in template.initiatives 
    ...
    def test_financial_services_projects(self, defaults_generator, sample_organizations):
        ...
        compliance_project = next(proj for proj in template.projects 
    ...
    def test_healthcare_hipaa_fields(self, defaults_generator, sample_organizations):
        ...
        clinical_team = next(team for team in template.teams if team.name == "Clinical Operations")
        ...
        privacy_team = next(team for team in template.teams if team.name == "Privacy & Compliance")
    ...
    def test_technology_sub_teams(self, defaults_generator, sample_organizations):
        ...
        platform_team = next(team for team in template.teams if team.name == "Platform Engineering")
    ...
    def test_technology_development_workflows(self, defaults_generator, sample_organizations):
    ...
    def test_technology_custom_fields(self, defaults_generator, sample_organizations):
        ...
        complexity_field = next(field for field in platform_team.custom_fields 
...
class TestSizeSpecificDefaults:
    ...
    def test_startup_defaults(self, defaults_generator, sample_organizations):
        ...
        org = sample_organizations['consulting_startup']  # Use consulting to avoid industry-specific teams
        template = defaults_generator.generate_defaults(org)
        
        # Startups should get an "All Hands" team if no other teams
        if not any(team.name != "All Hands" for team in template.teams):
            all_hands = next(team for team in template.teams if team.name == "All Hands")
    ...
    def test_startup_initiatives(self, defaults_generator, sample_organizations):
        ...
        initiative_names = [init.name for init in template.initiatives]
        pmf_initiatives = [name for name in initiative_names if "Market Fit" in name or "MVP" in name]
        assert len(pmf_initiatives) > 0
    
    def test_enterprise_matrix_organization(self, defaults_generator, sample_organizations):
        ...
        ea_team = next(team for team in template.teams if team.name == "Enterprise Architecture")
...
class TestSymphonyIntegration:
    ...
    def test_symphony_integration_basic_structure(self, defaults_generator, sample_organizations):
    ...
    def test_agent_assignments_match_teams(self, defaults_generator, sample_organizations):
        ...
        for org_key, org in sample_organizations.items():
            ...
            assert assigned_teams.issubset(team_names), f"Mismatch in {org_key}: {assigned_teams - team_names}"
    
    def test_size_based_agent_assignments(self, defaults_generator, sample_organizations):
    ...
    def test_enterprise_advanced_automation(self, defaults_generator, sample_organizations):
...
class TestDogfoodingTemplate:
    ...
    def test_dogfooding_meta_consistency(self, defaults_generator):
        ...
        assert any(word in description.lower() for word in ["uses", "manage", "development"])
    ...
    def test_team_key_consistency(self, defaults_generator, sample_organizations):
        ...
        for org_key, org in sample_organizations.items():
            ...
            for team in template.teams:
                ...
                assert team.key.isupper(), f"Team key '{team.key}' not uppercase in {org_key}"
                assert len(team.key) > 0, f"Empty team key in {org_key}"
                assert team.key != team.key.replace("_", " "), f"Team key should use underscores: {team.key}"
    
    def test_workflow_position_consistency(self, defaults_generator, sample_organizations):
        ...
        for org_key, org in sample_organizations.items():
            ...
            for team in template.teams:
                ...
                assert len(set(positions)) == len(positions), f"Duplicate positions in {org_key} team {team.name}"
                
                if positions:  # If there are workflows
                    assert min(positions) >= 0, f"Negative position in {org_key} team {team.name}"
