...
class TestDogfoodingValidation:
    ...
    def test_dogfooding_template_is_valid(self, defaults_generator, validator):
        ...
        assert (
            len([error for error in result.errors if "missing" in error.lower()]) == 0
        )

    def test_dogfooding_complexity_appropriate(
        self, defaults_generator, preview_generator
    ):
    ...
    def test_dogfooding_symphony_agents_deployed(
        self, defaults_generator, preview_generator
    ):
...
class TestDogfoodingConfigFile:
    ...
    def test_dogfood_config_validation(
        self, template_engine, validator, dogfood_config_path
    ):
        ...
        if dogfood_config_path.exists():
            ...
            ), f"Critical errors in dogfood config: {critical_errors}"
...
class TestDogfoodingIntegration:
    ...
    def test_dogfooding_template_saves_and_loads(
        self, defaults_generator, template_engine
    ):
    ...
    def test_dogfooding_variable_substitution(
        self, defaults_generator, template_engine
    ):
        ...
        variables = template_engine._build_variable_context(
...
class TestDogfoodingEdgeCases:
    ...
    def test_multiple_dogfood_generation_consistent(self, defaults_generator):
        ...
        assert (
            template1.organization.customer_name == template2.organization.customer_name
        )
...
class TestDogfoodingDocumentation:
    ...
    def test_dogfood_config_has_documentation(self, dogfood_config_path):
        ...
        if dogfood_config_path.exists():
            ...
            assert (
                "dogfood" in content.lower() or "meta" in content.lower()
            )  # Explains concept
