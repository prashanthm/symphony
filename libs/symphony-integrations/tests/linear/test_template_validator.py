...
from symphony_integrations.linear.template_models import (
    ...
    IndustryType, OrganizationSize, FieldType, TemplateValidationResult
)
...
def valid_template():
    ...
    initiatives = [
        Initiative(
            ...
            sub_initiatives=[
                Initiative("Code Quality", 2, "Improve code quality")
            ]
        )
    ]
    ...
    return WorkspaceTemplate(
        workspace={"name": "Test Workspace", "description": "Test workspace for validation"},
        organization=org,
        teams=teams,
        projects=projects,
...
class TestSchemaValidation:
    ...
    @patch('symphony_integrations.linear.template_validator.jsonschema.validate')
    def test_schema_validation_error_handling(self, mock_validate, validator, valid_template):
        ...
        assert result.is_valid == False
        assert any('schema validation error' in error.lower() for error in result.errors)
...
class TestBusinessRulesValidation:
    ...
    def test_duplicate_project_template_names(self, validator, valid_template):
        ...
        assert result.is_valid == False
        assert any('duplicate' in error.lower() and 'template name' in error.lower() 
...
class TestLinearConstraintsValidation:
    ...
    def test_large_number_of_teams_warning(self, validator):
        ...
        assert any('large number of teams' in warning.lower() for warning in result.warnings)
    ...
    def test_large_number_of_projects_warning(self, validator):
        ...
        assert any('large number of projects' in warning.lower() for warning in result.warnings)
    ...
    def test_custom_fields_limit_warning(self, validator):
        ...
        assert any('many custom fields' in warning.lower() for warning in result.warnings)
    ...
    def test_workflow_states_limit_warning(self, validator):
        ...
        assert any('many workflow states' in warning.lower() for warning in result.warnings)
...
class TestSymphonyIntegrationValidation:
    ...
    def test_missing_symphony_integration_suggestion(self, validator):
        ...
        template = WorkspaceTemplate(
            workspace={"name": "Test"},
            teams=[TeamTemplate("Test Team", "TEST")]
        )
        
        result = validator.validate_template(template)
        
        assert any('symphony integration' in suggestion.lower() 
...
class TestHierarchyDepthValidation:
    ...
    def test_team_hierarchy_depth_validation(self, validator):
        ...
        assert result.is_valid == False
        assert any('hierarchy depth' in error and 'exceeds maximum' in error 
                  for error in result.errors)
...
class TestBestPracticesSuggestions:
    ...
    def test_single_team_suggestion(self, validator):
        ...
        template = WorkspaceTemplate(
            workspace={"name": "Test"},
            teams=[TeamTemplate("Only Team", "ONLY")]
        )
        
        result = validator.validate_template(template)
        
        assert any('multiple teams' in suggestion.lower() for suggestion in result.suggestions)
    ...
    def test_no_initiatives_suggestion(self, validator, valid_template):
        ...
        assert any('initiatives' in suggestion.lower() for suggestion in result.suggestions)
    ...
    def test_no_projects_suggestion(self, validator, valid_template):
        ...
        assert any('project templates' in suggestion.lower() for suggestion in result.suggestions)
    ...
    def test_projects_without_milestones_suggestion(self, validator, valid_template):
        ...
        assert any('milestones' in suggestion.lower() for suggestion in result.suggestions)
...
class TestWorkspacePreviewGenerator:
    ...
    def test_preview_includes_sub_initiatives(self, preview_generator):
        ...
        main_initiative = Initiative("Main Initiative", 1, sub_initiatives=[sub_initiative])
        
        template = WorkspaceTemplate(
            workspace={"name": "Test"},
            initiatives=[main_initiative]
        )
        ...
        complex_template = WorkspaceTemplate(
            ...
            teams=[
                TeamTemplate(
                    name=f"Team {i}",
                    key=f"T{i}",
                    custom_fields=[CustomField(f"Field {j}", FieldType.TEXT) for j in range(5)],
                    sub_teams=[TeamTemplate(f"Sub {i}-{k}", f"S{i}{k}") for k in range(2)]
                ) for i in range(5)
            ],
            initiatives=[
                Initiative(
