...
📁 MONOREPO STRUCTURE - NEVER VIOLATE
12-Tier Universal Command Center
symphony/                           # THE Universal Command Center
├── 🚀 workspace/                  # Active development workspace (.claude settings)
├── 🏭 build/                      # Build system & automation
├── 🎼 core/                       # Core Symphony platform (was platform/)
│   ├── agents/                    # 85+ agent ecosystem (Maestro, Platform, Enterprise)
│   ├── orchestration/             # Coordination patterns & handoff protocols
│   ├── governance/                # Standards, compliance, security boundaries
│   ├── load-agents.sh            # Load agents into Claude Code
│   ├── validate-organization.sh  # Organization integrity validation
│   └── health-monitor.sh         # Platform health monitoring
...
├── 📦 releases/                   # Compiled releases & packages
├── 🛠️ tools/                     # Development & deployment tools
│   ├── org-builder/              # Organization compilation
│   ├── deployer/                 # Customer deployment automation
│   ├── docs-publisher/           # Documentation publishing pipeline
│   ├── validator/                # Multi-level validation
│   ├── monitor/                  # Health monitoring
│   ├── cli/                      # CLI utilities
│   └── symphony                  # Universal CLI (YOUR SINGLE COMMAND)
├── 📚 docs/                       # Multi-audience documentation
│   ├── internal/                 # Development team (autonomous-enterprise, conversations)
│   ├── external/                 # Customer-facing documentation
│   ├── api/                      # API documentation & references
│   ├── partnerships/             # Partner & integration documentation
│   └── symphony-structure-guide.md # MANDATORY REFERENCE (updated path)
├── 🔧 templates/                  # Enhanced development templates
├── 🌍 solutions/                  # Enhanced industry solutions
...
👨‍💻 Platform Developer
Entry Point: /workspace/ Primary Commands:
...
Entry Point: /organizations/ Primary Commands:
...
💼 Business Executive
Entry Point: /business/ Primary Commands:
...
Entry Point: /docs/ Primary Commands:
...
🔧 Operations Team
Entry Point: /ops/ Primary Commands:
...
Structure Integrity
NEVER create directories outside the 12-tier monorepo structure
ALWAYS use the Universal CLI (./tools/symphony) for operations
NEVER break the documentation hierarchy (internal/external/api/partnerships)
ALWAYS maintain agent directory standards in /core/agents/
ALWAYS verify links work after any file moves
Documentation Hierarchy
Internal Documentation → /docs/internal/ (development team)
External Documentation → /docs/external/ (customers)
API Documentation → /docs/api/ (integrations)
Partnership Documentation → /docs/partnerships/ (partners)
Conversation Management
ALWAYS save important conversations to /docs/internal/conversations/
Use timestamped format: [topic]-[YYYYMMDD-HHMMSS].md
Include complete context: requirements, decisions, outcomes
Customer Operations
Customer organizations → /organizations/customers/[customer-id]/
Default packages → /organizations/defaults/[type]/
Business data → /business/ and /data/
📋 MANDATORY REFERENCES
First Load Always
CRITICAL: Always load docs/symphony-structure-guide.md first in every Claude session. This contains the complete organizational structure and must never be violated.
...
Symphony agents will manage Symphony's business operations
Symphony orchestration will coordinate Symphony development
Symphony automation will deploy Symphony to customers
Symphony analytics will optimize Symphony performance
Symphony documentation will explain Symphony to the world
